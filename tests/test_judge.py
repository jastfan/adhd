                ...
                json.dumps(
                    {
                        "stub": {
                            ...
                            "command": [sys.executable, "-c", f"import sys, pathlib; p=pathlib.Path({repr(str(captured))}); p.write_text(sys.stdin.read(), encoding='utf-8'); sys.stdout.write(pathlib.Path({repr(str(payload))}).read_text(encoding='utf-8'))"],
                ...
                json.dumps(
                    {
                        "stub": {
                            "command": [
                                sys.executable,
                                "-c",
                                f"import sys, pathlib; p=sys.stdin.read(); sys.stdout.write('{{\"A\":{{\"correctness\":3}},\"B\":{{\"correctness\":3}}}}' if 'casual-message' in p else pathlib.Path({repr(str(good))}).read_text(encoding='utf-8'))",
                            ],
                            "response_format": "text",
                        }
                    }
                ...
                json.dumps(
                    {
                        "stub": {
                            "command": [
                                sys.executable,
                                "-c",
                                f"import sys, pathlib; p=sys.stdin.read(); sys.exit(7) if 'direct-answer' in p else sys.stdout.write(pathlib.Path({repr(str(verdict))}).read_text(encoding='utf-8'))",
                            ],
                            "response_format": "text",
                        }
                    }
