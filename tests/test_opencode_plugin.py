...
class OpenCodePluginTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # The plugin reads its flag from $XDG_CONFIG_HOME/opencode/.adhd-mode-always.
    ...
    def run_plugin(self, mode=None, config=None):
        ...
        args = [
            ...
            str(self.plugin_root / ".opencode" / "plugins" / "adhd-mode.mjs"),
        ]
        if mode:
            args.append(mode)
    ...
    def opt_in(self):
        (self.config_dir / "opencode" / ".adhd-mode-always").touch()

    def write_skill(self, text):
        (self.plugin_root / "skills" / "adhd-mode" / "SKILL.md").write_text(text)
    ...
    def test_config_preserves_existing_command(self):
        ...
        result = self.run_plugin("config", {"command": {"adhd-mode": custom}})
        self.assertEqual(0, result.returncode, result.stderr)
        self.assertEqual(custom, json.loads(result.stdout)["command"]["adhd-mode"])
    ...
    def test_command_preserves_metadata_and_trims_template(self):
        ...
        command = self.plugin_root / ".opencode/command/adhd-mode.md"
        command.write_bytes(("---  \r\n" + json.dumps(metadata) +
                             "\r\n--- \t\r\n\r\nUse the skill.\r\n\r\n").encode())
        result = self.run_plugin("config")
        self.assertEqual(0, result.returncode, result.stderr)
        self.assertEqual({**metadata, "template": "Use the skill."},
                         json.loads(result.stdout)["command"]["adhd-mode"])

    def test_missing_command_keeps_skill_discovery(self):
        (self.plugin_root / ".opencode/command/adhd-mode.md").unlink()
        result = self.run_plugin("config")
        self.assertEqual(0, result.returncode, result.stderr)
        config = json.loads(result.stdout)
        self.assertNotIn("adhd-mode", config["command"])
