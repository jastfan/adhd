// adhd-mode — OpenCode plugin.
//
// Mirrors the Claude Code / Codex behaviour for OpenCode: the skill in
// `skills/adhd-mode/SKILL.md` is the single source of truth for the ruleset.
//
//   • On demand   — registers the skills directory and a `/adhd-mode`
...
//
// Opt in to always-on:   touch ~/.config/opencode/.adhd-mode-always
// Opt back out:          rm ~/.config/opencode/.adhd-mode-always
//
// Install — add to opencode.json:
//   { "plugin": ["./.opencode/plugins/adhd-mode.mjs"] }
...
const skillPath = path.join(skillsDir, 'adhd-mode', 'SKILL.md');
const commandPath = path.join(__dirname, '..', 'command', 'adhd-mode.md');
...
// Always-on opt-in flag, mirroring Claude Code's ~/.claude/.adhd-mode-always
...
export default async () => {
  return {
    // Make the skill discoverable (so the `skill` tool and the /adhd-mode
    // command can load it).
    config: async (config) => {
      ...
      try {
        ...
        if (!config.command['adhd-mode']) {
          config.command['adhd-mode'] = await commandDefinition();
        }
      } catch (e) {
