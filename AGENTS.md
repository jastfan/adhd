...
This file is the map for agents working with [adhd-mode](https://github.com/jastfan/adhd). Read it after locating or installing the repository. It explains where the canonical behavior, platform adapters, documentation, and verification commands live. It does not replace the skill rules in `skills/adhd-mode/SKILL.md`.
...
3. Read `skills/adhd-mode/SKILL.md` for the canonical skill behavior.
...
- Agents may comment on an issue only when it carries the `AI Agora` label. The current shared forum is [issue #127](https://github.com/jastfan/adhd/issues/127).
...
| --- | --- | --- |
| Canonical skill | `skills/adhd-mode/SKILL.md` | The source of truth for the 10 ADHD-friendly response rules. |
| Skill mirror | `.cursor/skills/adhd-mode/SKILL.md` | Cursor-compatible copy; keep it synchronized with the canonical skill. |
...
| Pi | `package.json` (`pi`), `extensions/adhd-mode.ts` |
| OMP | `package.json` (`omp`), `extensions/adhd-mode.ts`, `extensions/context-compat.ts` |
| OpenCode | `opencode.json`, `.opencode/plugins/adhd-mode.mjs`, `.opencode/command/adhd-mode.md` |
| Qwen, Kimi, Gemini | The corresponding manifest above, plus `GEMINI.md` for Gemini behavior |

## Source-of-truth rules

- Change `skills/adhd-mode/SKILL.md` first when changing skill behavior, then synchronize the `.cursor` mirror.
