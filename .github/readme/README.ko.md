...
10가지 규칙. 전문은 [SKILL.md](../../skills/adhd-mode/SKILL.md)에 있습니다.
...
저장소를 포크해 `skills/adhd-mode/SKILL.md`를 수정한 다음, 본인 복사본으로 교체하세요:

```bash
claude plugin uninstall adhd-mode            # 먼저 업스트림 버전 제거
claude plugin marketplace remove adhd-mode   # 포크와 업스트림이 같은 이름을 씁니다
claude plugin marketplace add <jastfan>/adhd-mode
claude plugin install adhd-mode@adhd-mode
```

Claude Code를 재시작한 뒤 `/adhd-mode`를 다시 호출하세요.
