...
설치된 상태로 비활성화하려면 `agy plugin disable adhd-mode`를 실행하세요.
...
설치된 상태로 비활성화하려면 `claude plugin disable adhd-mode`를 실행하세요.

### 항상 활성화(선택 사항)

`SessionStart` 훅이 매 세션 시작 시 전체 규칙을 불러오므로 `/adhd-mode`를 입력할 필요가 없습니다:
...
`$adhd-mode`를 명시적으로 입력해 활성화하세요. Codex는 이 스킬을 자동으로 호출하지 않습니다.
...
새 세션을 시작하고 `/adhd-mode`를 입력하세요. 해당 세션 동안 활성화됩니다.
...
또는 세션에서 `/`를 입력하고 `adhd-mode`가 목록에 있는지 확인하세요.

### 업데이트

```bash
gemini extensions update adhd-mode    # 확장 프로그램 방식
# 명령 방식: 위 curl 다시 실행
```
...
```bash
gemini extensions uninstall adhd-mode    # 확장 프로그램 방식
...
```bash
npx skills add jastfan/adhd -a github-copilot        # 이 프로젝트
npx skills add jastfan/adhd -a github-copilot -g     # 모든 프로젝트
```
...
채팅 입력란에 `/`를 입력하고 `adhd-mode`가 나타나는지 확인하세요. 또는:
...
또는 설치된 skills 디렉터리에서 `adhd-mode` 폴더를 삭제하세요.

### 활성화 참고 사항

Copilot은 `disable-model-invocation`을 따릅니다. Claude Code와 마찬가지로 스킬을 호출하기 전에는 아무 규칙도 적용되지 않습니다([#60](https://github.com/jastfan/adhd/pull/60)에서 테스트).
...
`/adhd-mode`를 입력하세요. The skill installs into `~/.hermes/skills/` and is exposed as a slash command at the next session start.
...
tap도 제거하려면 `hermes skills tap remove jastfan/adhd`를 실행하세요.
...
1. `/plugins`를 실행합니다.
2. **Custom**을 선택합니다.
3. `https://github.com/jastfan/adhd`를 붙여넣고 Enter를 누릅니다.
4. **Trust and install**을 선택합니다.

slash 명령 `/skill:adhd-mode`로 이 스킬을 명시적으로 호출하세요.
...
새 세션을 시작하고 `/skill:adhd-mode`를 입력하세요.
...
또는 세션에서 `/skill:`을 입력하고 `adhd-mode`가 목록에 있는지 확인하세요.
...
또는 `~/.pi/agent/skills/adhd-mode`를 삭제하세요.
...
`/adhd-mode`를 입력해 이 스킬을 명시적으로 호출하세요. 확장만 설치해도 스킬을 호출하기 전까지 출력은 바뀌지 않습니다.
...
모든 프로젝트에서 사용하려면 **User** 범위에, 한 프로젝트에서만 사용하려면 **Project** 범위에 저장하세요. 그런 다음 Agent Panel에서 `/adhd-mode`를 입력하세요.
...
Agent Panel에서 Skills 관리자를 열어 `adhd-mode`가 목록에 있는지 확인하세요. 또는 `/`를 입력해 나타나는지 확인하세요.
...
Skills 관리자에서 `adhd-mode`를 제거하거나 `~/.agents/skills/adhd-mode`를 삭제하세요.
...
```bash
npx skills remove adhd-mode
npx skills remove adhd-mode -g    # 전역 설치한 경우
```
...
2. **명시적으로 호출합니다.** Claude Code와 Qwen Code에서는 `/adhd-mode`를, Codex에서는 `$adhd-mode`를 입력합니다. 해당 세션에서 규칙이 활성화됩니다. "stop adhd mode" 또는 "normal mode"로 끌 수 있습니다.
3. **`~/.claude/.adhd-mode-always`를 만듭니다**(Claude Code). `SessionStart` 훅이 매 세션의 첫 메시지부터 전체 규칙을 불러옵니다.
...
**자동 완성에 `/adhd-mode`가 없습니다.** 에이전트를 다시 시작하세요. 플러그인 인덱스는 시작 시 읽힙니다.

**항상 활성화 플래그가 작동하지 않습니다.** 플러그인을 업데이트하고(`claude plugin marketplace update adhd-mode`) 다시 시작하세요. 훅은 시작 시 읽히며 플래그에는 `hooks/hooks.json`이 포함된 플러그인 버전이 필요합니다.

**`claude plugin marketplace add`가 실패합니다.** `owner/repo` 형식을 사용하세요. 로컬 경로는 `.claude-plugin/`이 아니라 저장소 루트를 가리켜야 합니다.

**설치했지만 답변에 여전히 서론이 있습니다.** 새 세션을 여세요. 계속 어긋난다면 `skills/adhd-mode/SKILL.md`의 표현을 더 엄격하게 바꾸세요.

**다른 규칙을 원합니다.** 저장소를 포크하고 `skills/adhd-mode/SKILL.md`를 수정한 뒤 자신의 복사본으로 교체하세요:

```bash
claude plugin uninstall adhd-mode            # 먼저 업스트림 복사본 제거:
claude plugin marketplace remove adhd-mode   # 포크와 업스트림은 같은 이름 사용
claude plugin marketplace add <jastfan>/adhd-mode
claude plugin install adhd-mode@adhd-mode
```

다시 시작한 뒤 `/adhd-mode`를 다시 호출하세요.
