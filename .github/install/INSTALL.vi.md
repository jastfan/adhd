...
Hoặc giữ nguyên cài đặt và tắt bằng `agy plugin disable adhd-mode`.
...
Hoặc giữ nguyên cài đặt và tắt bằng `claude plugin disable adhd-mode`.

### Luôn bật (không bắt buộc)

Hook `SessionStart` tải toàn bộ bộ quy tắc khi bắt đầu mỗi phiên, không cần `/adhd-mode`:
...
Gọi skill một cách rõ ràng bằng cách gõ `$adhd-mode`. Codex sẽ không tự động kích hoạt skill.
...
Bắt đầu phiên mới và gõ `/adhd-mode`. Skill sẽ bật trong suốt phiên đó.
...
Hoặc gõ `/` trong một phiên và xác nhận `adhd-mode` có trong danh sách.

### Cập nhật

```bash
gemini extensions update adhd-mode    # cách dùng extension
...
```bash
gemini extensions uninstall adhd-mode    # cách dùng extension
...
```bash
npx skills add jastfan/adhd -a github-copilot        # dự án này
npx skills add jastfan/adhd -a github-copilot -g     # mọi dự án
```
...
Gõ `/` trong ô chat và xác nhận `adhd-mode` xuất hiện. Hoặc:
...
Hoặc xóa thư mục `adhd-mode` khỏi thư mục skills nơi nó được cài.

### Lưu ý về kích hoạt

Copilot tuân theo `disable-model-invocation`: không có gì được áp dụng cho đến khi bạn gọi skill, giống Claude Code (đã kiểm thử trong [#60](https://github.com/jastfan/adhd/pull/60)).
...
Gõ `/adhd-mode`. The skill installs into `~/.hermes/skills/` and is exposed as a slash command at the next session start.
...
Hoặc xóa cả tap bằng `hermes skills tap remove jastfan/adhd`.
...
1. Chạy `/plugins`.
2. Chọn **Custom**.
3. Dán `https://github.com/jastfan/adhd` rồi nhấn Enter.
4. Chọn **Trust and install**.

Dùng lệnh slash `/skill:adhd-mode` để gọi skill một cách rõ ràng.
...
Bắt đầu phiên mới và gõ `/skill:adhd-mode`.
...
Hoặc gõ `/skill:` trong một phiên và xác nhận `adhd-mode` có trong danh sách.
...
Hoặc xóa `~/.pi/agent/skills/adhd-mode`.
...
Gõ `/adhd-mode` để gọi skill một cách rõ ràng. Chỉ cài extension sẽ không thay đổi đầu ra cho đến khi skill được gọi.
...
Xác nhận `adhd-mode` xuất hiện trong danh sách.
...
Lưu ở phạm vi **User** cho mọi dự án hoặc **Project** cho một dự án. Sau đó gõ `/adhd-mode` trong Agent Panel.
...
Mở trình quản lý Skills trong Agent Panel và xác nhận `adhd-mode` có trong danh sách. Hoặc gõ `/` và xác nhận nó xuất hiện.
...
Xóa `adhd-mode` khỏi trình quản lý Skills, hoặc xóa `~/.agents/skills/adhd-mode`.
...
```bash
npx skills remove adhd-mode
npx skills remove adhd-mode -g    # nếu đã cài toàn cục
```
...
2. **Bạn gọi skill một cách rõ ràng.** Gõ `/adhd-mode` trong Claude Code hoặc Qwen Code, hoặc `$adhd-mode` trong Codex. Quy tắc bật trong phiên đó. "stop adhd mode" hoặc "normal mode" sẽ tắt chúng.
3. **Bạn tạo `~/.claude/.adhd-mode-always`** (Claude Code). Hook `SessionStart` tải toàn bộ quy tắc từ tin nhắn đầu tiên trong mọi phiên.
...
**`/adhd-mode` không có trong tự động hoàn thành.** Khởi động lại agent. Chỉ mục plugin được đọc khi khởi động.

**Cờ luôn bật không có tác dụng.** Cập nhật plugin (`claude plugin marketplace update adhd-mode`) và khởi động lại. Hook được đọc khi khởi động, và cờ cần phiên bản plugin có `hooks/hooks.json`.
...
**Đã cài nhưng phản hồi vẫn có lời mở đầu.** Mở phiên mới. Nếu vẫn lệch, hãy làm chặt hơn cách diễn đạt trong `skills/adhd-mode/SKILL.md`.

**Muốn quy tắc khác.** Fork repo, sửa `skills/adhd-mode/SKILL.md`, rồi chuyển sang bản của bạn:

```bash
claude plugin uninstall adhd-mode            # gỡ bản upstream trước:
claude plugin marketplace remove adhd-mode   # fork và upstream dùng cùng tên
claude plugin marketplace add <jastfan>/adhd-mode
claude plugin install adhd-mode@adhd-mode
```

Khởi động lại rồi gọi `/adhd-mode` lần nữa.
