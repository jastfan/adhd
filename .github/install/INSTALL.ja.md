...
インストールしたまま無効化する場合：`agy plugin disable adhd-mode`
...
インストールしたまま無効化する場合：`claude plugin disable adhd-mode`

### 常時有効（任意）

`SessionStart` フックが毎セッション開始時に完全なルールセットを読み込むため、`/adhd-mode` の入力が不要になります：
...
`$adhd-mode` と明示的に入力してスキルを呼び出します。Codex が自動で有効化することはありません。
...
新しいセッションを開始して `/adhd-mode` と入力します。そのセッション中は有効な状態が続きます。
...
ls ~/.gemini/commands           # コマンド方式: adhd-mode.toml が存在することを確認
```

または、セッション内で `/` と入力し、一覧に `adhd-mode` が表示されることを確認します。

### 更新

```bash
gemini extensions update adhd-mode    # 拡張機能方式
...
```bash
gemini extensions uninstall adhd-mode    # 拡張機能方式
rm ~/.gemini/commands/adhd-mode.toml     # コマンド方式
```
...
```bash
npx skills add jastfan/adhd -a github-copilot        # このプロジェクトのみ
npx skills add jastfan/adhd -a github-copilot -g     # 全プロジェクト共通
```
...
チャット入力欄に `/` と入力し、`adhd-mode` が表示されることを確認します。または：
...
または、配置先の skills ディレクトリから `adhd-mode` フォルダーを削除します。

### 有効化に関する注意

Copilot は設定項目の `disable-model-invocation` に従います。Claude Code と同様に、スキルを明示的に呼び出すまでルールは適用されません（[#60](https://github.com/jastfan/adhd/pull/60) で検証済み）。
...
`/adhd-mode` と入力します。スキルは `~/.hermes/skills/` にインストールされ、次回のセッション開始時からスラッシュコマンドとして利用可能になります。
...
tap も削除する場合：`hermes skills tap remove jastfan/adhd`
...
1. `/plugins` を実行する。
2. **Custom** を選択する。
3. `https://github.com/jastfan/adhd` を貼り付けて `Enter` を押す。
4. **Trust and install** を選択する。

スラッシュコマンド `/skill:adhd-mode` を使用することで、スキルを明示的に呼び出します。
...
OpenCode はこのリポジトリをサーバープラグインとして読み込みます。`.opencode/plugins/adhd-mode.mjs` が `skills/` のエントリーポイントと `/adhd-mode` コマンドを登録し、常時有効化されている場合はルールセットを注入します。また、OpenCode は `skills/` をネイティブにも読み込めるため、プラグインなしでもスキル自体は動作します（プラグインを追加すると `/adhd-mode` コマンドと常時有効化フラグが使えるようになります）。
...
OpenCode を起動して `/` と入力し、コマンド一覧に `adhd-mode` が表示されることを確認します。
...
GitHub パッケージが一覧にあることを確認し、`/adhd-mode` と入力してフッターに `● ADHD ON` が表示されることを確認します。
...
Pi のエージェント設定ディレクトリに `~/.pi/agent/adhd-mode.json` を作成します：
...
- `alwaysOn`：すべてのセッションをルール有効の状態で開始します。従来の `.adhd-mode-always` フラグファイルも引き続き使えます。
- `hideStatus`：ステータスバーの `● ADHD ON` 表示を隠します。ルールと `/adhd-mode` コマンドは引き続き機能します。
...
`PI_CODING_AGENT_DIR` が設定されている場合は、そのディレクトリ内に `.adhd-mode-always` を配置してください。フラグを変更した後は `/reload` を実行するか、新しいセッションを開始してください。
...
新しい OMP セッションを開始し、`/adhd-mode` を実行してモードを切り替えます。
...
スキルを明示的に呼び出すには `/adhd-mode` と入力します。拡張機能をインストールしても、スキルを呼び出すまで出力の変化はありません。
...
全プロジェクト共通で使う場合は **User** スコープ、現在のプロジェクトのみなら **Project** スコープで保存します。その後、Agent Panel で `/adhd-mode` と入力します。
...
Agent Panel で Skills マネージャーを開き、`adhd-mode` が一覧にあることを確認します。または、`/` と入力して表示されることを確認します。
...
Skills マネージャーから `adhd-mode` を削除するか、`~/.agents/skills/adhd-mode` を削除します。
...
```bash
npx skills add jastfan/adhd                  # このプロジェクトのみ
npx skills add jastfan/adhd -g               # 全プロジェクト共通
npx skills add jastfan/adhd -a cursor -y     # 特定のエージェントのみ
npx skills add jastfan/adhd -a opencode -y
```

新しいエージェントチャットで `/adhd-mode` と入力します。
...
```bash
npx skills remove adhd-mode
npx skills remove adhd-mode -g    # グローバルにインストールした場合
```
...
2. **明示的に呼び出す**：Claude Code や Qwen Code では `/adhd-mode`、Codex では `$adhd-mode` と入力します。そのセッション中はルールが有効になります。`stop adhd mode` または `normal mode` でオフにできます。
3. **`~/.claude/.adhd-mode-always` を作成する**（Claude Code）：`SessionStart` フックにより、すべてのセッションで最初のメッセージから完全なルールセットが読み込まれます。
...
**`/adhd-mode` が自動補完に表示されない →** エージェントを再起動してください。プラグインのインデックスは起動時に読み込まれます。

**常時有効フラグが効かない →** プラグインを更新（`claude plugin marketplace update adhd-mode`）して再起動してください。フックは起動時に読み込まれるほか、フラグの認識には `hooks/hooks.json` が含まれるバージョンが必要です。

**`claude plugin marketplace add` が失敗する →** `owner/repo` 形式を使用してください。ローカルパスを指定する場合は `.claude-plugin/` ではなくリポジトリのルートを指す必要があります。

**インストールしたのに回答に前置きが入る →** 新しいセッションを開いてください。それでも指示から逸脱する場合は、`skills/adhd-mode/SKILL.md` の文言をより厳格に調整してください。

**自分好みのルールに変更したい →** Fork して `skills/adhd-mode/SKILL.md` を編集し、自分のコピーに差し替えます：

```bash
claude plugin uninstall adhd-mode            # 先に本家のコピーを削除
claude plugin marketplace remove adhd-mode   # Fork版と本家で名前が重複するため
claude plugin marketplace add <jastfan>/adhd-mode
claude plugin install adhd-mode@adhd-mode
```

再起動してから、`/adhd-mode` をもう一度呼び出してください。
