...
也可以保留安装并将其关闭：`agy plugin disable adhd-mode`。
...
1. 下载[技能源文件 SKILL.md](https://raw.githubusercontent.com/jastfan/adhd/main/skills/adhd-mode/SKILL.md)，保存为 `SKILL.md`，上传前先阅读文件内容。
2. 在 AstronClaw 中打开**我的技能**，选择**新建**，上传该 `.md` 文件。
3. 确认导入后的技能名称为 `adhd-mode`，通过**启用/禁用**控制技能是否可用。
...
确认**我的技能**中出现 `adhd-mode`，通过**下载**将导入后的指令与原始技能对照检查，
然后启用并尝试输入：

```text
请在本次对话中使用 adhd-mode 技能。说明如何在新文件夹中创建一个空的 Git 仓库。
```
...
保存备份。如需完整替换，请删除旧的 `adhd-mode` 条目，重新导入文件，
并在新对话中运行验证提示词。

### 卸载

在**我的技能**中选择 `adhd-mode`，点击**删除**，然后开启新对话。
...
也可以保留安装并将其关闭：`claude plugin disable adhd-mode`。

### 始终启用（可选）

`SessionStart` 钩子会在每次会话开始时加载完整规则，无需输入 `/adhd-mode`：
...
明确输入 `$adhd-mode` 来启用此技能。Codex 不会自动调用它。
...
开始新会话并输入 `/adhd-mode`。它会在该会话中持续启用。
...
也可以在会话中输入 `/`，确认列表中有 `adhd-mode`。

### 更新

```bash
gemini extensions update adhd-mode    # 扩展方式
# 命令方式：重新运行上面的 curl
```
...
```bash
gemini extensions uninstall adhd-mode    # 扩展方式
...
```bash
npx skills add jastfan/adhd -a github-copilot        # 此项目
npx skills add jastfan/adhd -a github-copilot -g     # 所有项目
```
...
也可以从安装所在的 skills 目录删除 `adhd-mode` 文件夹。

### 启用说明

Copilot 遵循 `disable-model-invocation`：与 Claude Code 相同，在调用技能前不会应用任何规则（已在 [#60](https://github.com/jastfan/adhd/pull/60) 中测试）。
...
输入 `/adhd-mode`。 The skill installs into `~/.hermes/skills/` and is exposed as a slash command at the next session start.
...
也可以同时删除 tap：`hermes skills tap remove jastfan/adhd`。
...
1. 输入 `/plugins`。
2. 选择 **Custom**。
3. 粘贴 `https://github.com/jastfan/adhd` 并 Enter。
4. 选择 **Trust and install**。

使用斜杠命令 `/skill:adhd-mode` 显式调用此技能。
...
也可以在会话中输入 `/skill:`，确认列表中有 `adhd-mode`。
...
也可以删除 `~/.pi/agent/skills/adhd-mode`。
...
安装扩展本身不会改变输出，除非输入 `/adhd-mode` 显式调用此技能。
...
要用于所有项目，请保存到 **User** 作用域；仅用于一个项目则保存到 **Project** 作用域。然后在 Agent Panel 中输入 `/adhd-mode`。
...
在 Agent Panel 中打开 Skills 管理器，确认列表中有 `adhd-mode`。也可以输入 `/` 并确认它出现。
...
从 Skills 管理器中移除 `adhd-mode`，或删除 `~/.agents/skills/adhd-mode`。
...
```bash
npx skills remove adhd-mode
npx skills remove adhd-mode -g    # 如果全局安装
```
...
2. **明确调用技能。** 在 Claude Code 或 Qwen Code 中输入 `/adhd-mode`，在 Codex 中输入 `$adhd-mode`。规则将在该会话中启用。输入“stop adhd mode”或“normal mode”可将其关闭。
3. **创建 `~/.claude/.adhd-mode-always`**（Claude Code）。`SessionStart` 钩子会在每次会话中从第一条消息起加载完整规则。
...
**自动补全中没有 `/adhd-mode`。** 重启智能体。插件索引在启动时读取。

**始终启用标志无效。** 更新插件（`claude plugin marketplace update adhd-mode`）并重启。钩子在启动时读取，且该标志需要包含 `hooks/hooks.json` 的插件版本。

**`claude plugin marketplace add` 失败。** 使用 `owner/repo` 格式。本地路径必须指向仓库根目录，而不是 `.claude-plugin/`。

**已安装，但回复仍有开场白。** 开启新会话。如果仍然偏离，请收紧 `skills/adhd-mode/SKILL.md` 中的措辞。

**想使用不同规则。** Fork 仓库，编辑 `skills/adhd-mode/SKILL.md`，然后换成你的副本：

```bash
claude plugin uninstall adhd-mode            # 先移除上游副本：
claude plugin marketplace remove adhd-mode   # fork 与上游使用相同名称
