...
def validate_package_manifest() -> None:
    ...
    assert package.get("pi") == {
        "extensions": ["./extensions/adhd-mode.ts"],
        "skills": ["./skills"],
    }
...
def status_texts(events: list[dict[str, Any]]) -> list[str | None]:
    return [
        ...
        and event.get("statusKey") == "adhd-mode"
    ]
...
def latest_enabled(entries_response: dict[str, Any]) -> bool:
    states = [
        ...
        and entry.get("customType") == "adhd-mode-state"
    ]
    if not states or not isinstance(states[-1], bool):
        raise AssertionError("No persisted adhd-mode state found")
    return states[-1]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke-test the adhd-mode extension without a model request."
    )
    parser.add_argument(
        "--runtime",
        choices=("pi", "omp"),
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"adhd-mode-{args.runtime}-"
    ) as agent_dir:
...
"""
        ...
        if args.runtime == "omp":
            try:
                ...
                assert "adhd-mode" in command_names
                assert "skill:adhd-mode" in command_names
                assert any(
                    "ADHD ON" in (text or "")
        ...
        try:
            ...
            assert message_count(entries, "adhd-mode-rules") == 1
            assert message_count(entries, "adhd-mode-disabled") == 0
            ...
            assert message_count(entries, "adhd-mode-disabled") == 1
            ...
            assert message_count(entries, "adhd-mode-rules") == 2

            explicit_off, _ = client.request(
                "explicit-off",
                {"type": "prompt", "message": "/adhd-mode off"},
            )
            assert explicit_off["success"] is True

            skill, skill_events = client.request(
                "skill-alias",
                {"type": "prompt", "message": "/skill:adhd-mode"},
            )
            ...
            assert latest_enabled(entries) is True
            assert message_count(entries, "adhd-mode-rules") == 3
        ...
        if args.runtime == "pi":
            Path(agent_dir, ".adhd-mode-always").touch()
            always_on = RpcClient(
                executable,
                env,
                "--no-session",
